const parse = (string) => {
    ...
    const parseLiteral = (string) => {
        ...
        switch (string[0]) {
            ...
            case '$': // hex
                ...
                return hex
            case '\'':
                if (string[2] != '\'') throw new Error(`Unclosed char: ${string}`)
                return string.charCodeAt(1)
            default: // decimal or variable
    ...
    const parseInstruction = (mnemonic, arguments) => {
        ...
        if (arguments.split(' ').join('') == 0) {
            ...
            return tokenized[bytePointer ++] = parsed
        }
        ...
        const regexHell = [ // what the fuck. regex101.com is lowkey carrying
            ...
            },
            {
                ...
                type: 'zpg/abs_y' // zero page or absolute, y
            },
            {
                regex: /^\(([\$\%\w]+(?:\s*[\+\-\*\&\^\|]\s*[\$\%\w]+\w*)*)\)\s*$/,
                type: 'ind' // indirect
            ...
            if (captured != null) {
                ...
                switch(item.type) {
                    case 'a':
                        return addToken()
                    case 'imm':
                        return addToken(captured[1] == undefined ? captured[2] : captured[1], true, true)
                    case 'ind_x':
                    case 'ind_y':       
                    ...
                    case 'zpg/abs_y':
                        try {
                            ...
                            parsed.addressingMode = `abs${item.type.slice(7)}`      
                            return addToken(captured[1], true, false, false)
                        }
