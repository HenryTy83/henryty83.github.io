...
const tokenize = (lines = []) => {
    ...
    const parseNumber = (number = '', expectByte = false) => {
        ...
        switch (number[0]) {
            ...
            case '(': //expression... god help our souls
                // console.log(number.slice(1, findClosingParenthesis(number)))
                // console.log(parseExpression(number.slice(1, findClosingParenthesis(number))))
                return parseExpression(number.slice(1, findClosingParenthesis(number)))
        }
    }

    const findOperation = (string) => {
    ...
    const parseExpression = (string, start = 0) => {
        ...
        // console.log(string)
        if (string[0] == '(') {
            parsedExpression.arg1 = parseExpression(string.slice(1, findClosingParenthesis(string)))

            string = string.slice(findClosingParenthesis(string) + 1)
            if (string == '') throw new Error(`Expected an operation, recieved end of expression ${string}`)

            operator = string[0]
            if (!operations.includes(operator)) throw new Error(`Expected an operation, recieved something else? ${string}`)
        ...
        for (var i = 0; i < 10000; i++) { // if ur expression is this long something else is wrong
            ...
            if (string[0] == '(') {
                parsedExpression.arg2 = parseExpression(string.slice(1, findClosingParenthesis(string)))

                string = string.slice(findClosingParenthesis(string) + 1)
                if (string == '') return parsedExpression
            }
            ...
            string = string.slice(nextOperatorIndex)
            operator = string[0]
            string = string.slice(1)
    ...
    const parseLiteral = (literal, expectByte = false) => {
        ...
        if (parsedNumber != null) {
            if (parsedNumber.type == 'EXPRESSION') return parsedNumber
    ...
    const parseInstruction = (instruction) => {
        var parsedInstruction = {
            type: 'INSTRUCTION',
            value: null,
            mnemonic: instruction[0].toLowerCase(),
            argument: null
        }

        var arg = instruction.slice(1)[0]
        ...
        if (parsedInstruction.mnemonic[0] == 'b' && (parsedInstruction.mnemonic != 'brk' && parsedInstruction.mnemonic != 'bit')) {  // fking worst way to check for branch
            parsedInstruction.value = 'REL'
            instructionPointer++

            if (arg[0] == '*') { // hardcoded offset
                var offset = parseInt(arg.slice(1)) - 2
                if (isNaN(offset)) throw new Error(`Unknown branch offset: ${instruction}`)

                if (offset < 0) offset = ((-offset) | 0b10000000) & 0xffff
                if (offset > 0xff) throw new Error(`Branch offset too big: ${instruction}`)

                parsedInstruction.argument = {
                    type: 'BYTE',
                    value: offset & 0xff
                }
            ...
            var targetLocation = parseLiteral(arg)
            if (targetLocation.type != 'LABEL') {
                var offset = targetLocation.value - instructionPointer - 2
        ...
        if (arg[0] == '#') {
            ...
            parsedInstruction.argument = parseLiteral(arg.slice(1))

            // console.log(parsedInstruction, parsedInstruction.argument)
        ...
        if (arg[0] == '(') { // some kind of indirect
            var address = arg.split('(')[1].split(')')[0]
            if (arg.split(')')[1].length != 0) { // (zp),y
                parsedInstruction.value = 'IND_Y'

                parsedInstruction.argument = parseLiteral(address, true)
                instructionPointer++
        ...
        if (arg.split(',').length == 1) { // either WORD or BYTE
            var operand = parseLiteral(arg)
        ...
        if (arg.split(',').length > 1) { // index
            var address = parseLiteral(arg.split(',')[0])
            var register = arg.split(',')[1]
                ...
                for (var i = 0; i < string.length; i++) {
                    ...
                    if (inString) {
                        ...
                        if (char == '"') {
                            ...
                            if (literal != '') {
                                var extractedLiterals = literal.split(',')
                                extractedLiterals = extractedLiterals.slice(1, extractedLiterals.length - 1)
                                for (var extractedLiteral of extractedLiterals) {
                                    if (extractedLiteral.length == 0) throw new Error(`Empty string in ${string}`)
                                    else {
    ...
    const evaluate = (expression) => {
        if (expression.type == 'LABEL') {
            ...
            // console.log(expression.value, constants[expression.value])
            return {
        ...
        if (expression.type == 'BYTE' || expression.type == 'WORD') return {
            type: expression.type,
            value: expression.value & (expression.type == 'BYTE' ? 0xff : 0xffff)
        } // already defined

        // now we only have expressions 
        var value = expression.operation(evaluate(expression.arg1).value, evaluate(expression.arg2).value) & 0xffff
    ...
    for (var line of Object.keys(syntaxTree)) {
        // console.log(syntaxTree[line])

        if (syntaxTree[line].argument != undefined) {
            // console.log(syntaxTree[line].argument)
            syntaxTree[line].argument = evaluate(syntaxTree[line].argument);
            // console.log(syntaxTree[line].argument)
            // console.log('')
        }

        // 
        else if (syntaxTree[line].type == 'EXPRESSION') {
            // console.log(syntaxTree[line])
            syntaxTree[line] = evaluate(syntaxTree[line]);
            // console.log(syntaxTree[line])
            // console.log('')
        }
    }
