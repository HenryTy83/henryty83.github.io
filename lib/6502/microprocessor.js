class Microprocessor {PHP_I
    ...
    addLowByte(a, b) { return (a & 0xFF00) + (((0xFF & a) + (0xFF & b)) & 0xFF) }
    ...
    execute(opcode) {
        switch (opcode) {
            ...
            case Microprocessor.opcodes.LDA_IND_X:
                var address = this.addLowByte(this.fetch(), this.registers['X'].getUint8(0))
                var value = this.readMemory(address)
                this.updateNZ(value)
                this.registers['A'].setUint8(0, value)
                break;
