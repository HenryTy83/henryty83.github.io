...
class Microprocessor {
    ...
    branchIF(relative, condition) { 
        var PC = this.readPC()
        var signedOffset = (relative & 0b10000000) ? -(relative & 0b01111111) : relative
        var branch = (signedOffset + PC + 0x10000) & 0xffff
    ...
    execute(opcode) {
        ...
        switch (opcode) {
            ...
            case Microprocessor.opcodes.BPL_REL:
                this.branchIF(this.fetch(), !this.getFlag('n'))
                break;
            ...
            case Microprocessor.opcodes.JSR_WORD:
                ...
                this.push((this.readPC() & 0xff00) >> 8)
                this.push(this.readPC() & 0xff)
            ...
            case Microprocessor.opcodes.BVC_REL:
                this.branchIF(this.fetch(), !this.getFlag('v'))
                break;
            ...
            case Microprocessor.opcodes.RTS_I:
                // this.dump()
                this.writePC(this.pop() + (this.pop() << 8))
                // this.fetch()
                // this.dump()
                break;
            ...
            case Microprocessor.opcodes.BCC_REL:
                this.branchIF(this.fetch(), !this.getFlag('c'))
                break;
            ...
            case Microprocessor.opcodes.LDA_IND_Y:
                ...
                var addressLow = this.readMemory(address)
                var addressHigh = this.readMemory(this.addLowByte(address, 1))

                var indirectAddress = (addressHigh << 8) + addressLow

                this.writeReg(this.readMemory(this.addLowByte(indirectAddress, this.readReg('Y'))), 'A')
                break;
            ...
            case Microprocessor.opcodes.BNE_REL:
                this.branchIF(this.fetch(), !this.getFlag('z'))
                break;
