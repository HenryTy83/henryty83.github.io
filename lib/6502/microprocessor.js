...
class Microprocessor {
    ...
    addrIND_X() {
        return this.getIndirectAddress(this.fetchWord(), 'X', true)
    }
    ...
    addrIND_Y() {
        return this.readMemory(this.getIndirectAddress(this.getABS(), 'Y', false))
    }
    ...
    addrZPG_X() {
        return this.getIndirectAddress(this.fetch(), 'X', false)
    }
    ...
    addrZPG_Y() {
        return this.getIndirectAddress(this.fetch(), 'Y', true)
    }
    ...
    execute(opcode) {
        ...
        switch (opcode) {
            case Microprocessor.opcodes.BRK_I:
                ...
                var jumpTo = this.readMemory(0xfffe) + (this.readMemory(0xffff) << 8)
                this.writePC(jumpTo)
                this.setFlag('b', 1)
                break;
            ...
            case Microprocessor.opcodes.JMP_IND: // documented bug
                ...
                var jumpTo = this.readMemory(address) + (this.readMemory((address + 1) & 0xffff) << 8)
                this.writePC(jumpTo)
                break;
